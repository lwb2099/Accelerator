...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def _relative_position_multi_head_forward(self, q, k, v, key_padding_mask, attn_mask, max_relative_position=4):
        ...
        ----------
            ...
            key_padding_mask: [batch_size, src_len], mask keys that are `padding`
            attn_mask: [tgt_len, src_len]
        ----------
        Returns:
            attn_output: [tgt_len, batch_size, d_model]
            attn_output_weights: [batch, tgt_len, src_len]
        """
    ...
    def forward(
        ...
        if (
            ...
            if self.use_xformers:
                ...
                need_weights = True
                attn_val, attn_weight = F.multi_head_attention_forward(
                    query,
                    key,
                    value,
                    self.embed_dim,
