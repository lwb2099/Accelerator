...
class SummaCBenchmark:
    ...
    def load_cnndm(self, file_path, split="all"):
        # 读取n行，暂存为一个csv
        csv_data = {}
        """train"""
        if split == "all":
        ...
        """val"""
        if split in ["val", "all"]:
            split_path = file_path + "/split_val.csv"
            df = pd.read_csv(file_path + f"/val.csv", index_col="id")
            df.loc[df.index.isin(self.d)].to_csv(split_path)
            csv_data["val"] = split_path
        """test"""
        if split in ["test", "all"]:
            split_path = file_path + "/split_test.csv"
            df = pd.read_csv(file_path + f"/test.csv", index_col="id")
            df.loc[df.index.isin(self.d)].to_csv(split_path)
            csv_data["test"] = split_path
        logger.debug(f"loading {len(self.d)} data from split={split}")
        d = DatasetDict.from_csv(csv_data, keep_in_memory=True)
        return d
    ...
    def get_cnndm_reference(self, aid):
        global CNNDM
        if CNNDM is None:
            CNNDM = load_from_disk(dataset_path=self.path, keep_in_memory=True)
            self.cnndm = CNNDM
    ...
    def load_cogensumm(self):
        ...
        if not os.path.exists(dataset_folder):
            logger.info("==== CoGenSumm dataset not found, downloading from scratch")
        ...
        for fn in os.listdir(dataset_folder):
            ...
            if "_org" in fn or fn == "test_chen18_reranked.json":
                logger.debug(f"in fn={fn}: ")
                for aid in dataset:
                    document = self.get_cnndm_document(aid, split="test")
                ...
                logger.debug(f"in fn={fn}: ")
                self.d = list(dataset.keys())
                for aid in dataset:
                    document = self.get_cnndm_document(aid, split="val")
                ...
                logger.debug(f"in fn={fn}: ")
                self.d = [d["article_id"] for d in dataset]
                for d in dataset:  # dataset: [dict]
                    aid = d["article_id"]  # 一堆article_id
                    document = self.get_cnndm_document(aid, split="val")
                    clean_dataset.append(
    ...
    def load_xsumfaith(self):
        ...
        if not os.path.exists(dataset_folder):
            logger.info("==== XSum dataset not found, downloading from scratch")
            os.makedirs(dataset_folder)
    ...
    def load_polytope(self, which_label="overall"):
        ...
        if not os.path.exists(dataset_folder):
            logger.info("==== Polytope dataset not found, downloading from scratch")
            os.makedirs(dataset_folder)
    ...
    def load_summeval(self, key_focus="consistency"):
        ...
        if not os.path.exists(dataset_folder):
            logger.info("==== SummEval dataset not found, downloading from scratch")
            os.makedirs(dataset_folder)
    ...
    def load_frank(self):
        ...
        if not os.path.exists(dataset_folder):
            logger.info("==== Frank dataset not found, downloading from scratch")
            os.makedirs(dataset_folder)
    ...
    def print_stats(self):
        ...
        logger.debug(f"dataset_states = {pd.DataFrame(dataset_stats)}")
...
if __name__ == "__main__":
    ...
    for cut in ["val", "test"]:
        summac_benchmark = SummaCBenchmark(benchmark_folder="./datasets/process_summary/summac_benchmark/", cut=cut)
        logger.info("============= SUMMAC %s ===============" % (cut.upper()))
        summac_benchmark.print_stats()
        for dataset in summac_benchmark.datasets:
            logger.info("\n============= %s ===============" % (dataset["name"]))
            random.shuffle(dataset["dataset"])
            logger.info(f"document: {dataset['dataset'][0]['document'][:400]}")
            logger.info("-------------")
            logger.info(f"claim: {dataset['dataset'][0]['claim']}")
            logger.info("-------------")
            logger.info(f"label: dataset['dataset'][0]['label']")
