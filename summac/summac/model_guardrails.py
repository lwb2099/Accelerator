...
def doc2ents(doc, black_list_types=[]):
    ents = [{"type": ent.label_, "entity": ent.text, "sent_idx": sent_idx} for sent_idx, sent in enumerate(doc.sents)
...
class NERInaccuracyPenalty:
    def __init__(self, spacy_model="en_core_web_sm"):
        ...
        self.number_words_to_remove = set(
            ["the", "a", "an", "at", "of", "in", "than", "several", "few", "only", "about", "another", "least", "most",
             "last", "first", "early", "earlier", "later", "over", "fewer", "row", "every", "late", "ago", "only",
             "about", "around", "within", "more", "less"])

        self.string2digits = {"zero": 0, "one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7,
                              "eight": 8, "nine": 9, "ten": 10, "eleven": 11, "twelve": 12, "thirteen": 13,
                              "fourteen": 14, "fifteen": 15, "sixteen": 16, "seventeen": 17, "eighteen": 18,
                              "nineteen": 19, "twenty": 20, "thirty": 30, "forty": 40, "fifty": 50, "sixty": 60,
                              "seventy": 70, "eighty": 80, "ninety": 90, "a hundred": 100, "hundred": 100,
    ...
    def remove_common_entities(self, ent_list_new, ent_list_old, source_text):
        ...
        for ent_new in ent_list_new:
            ...
            if entity_text in ent_set or self.singular(entity_text) in ent_set:  # Exact match with some entity
                continue
            if entity_text in source_text or self.singular(
                    entity_text).lower() in source_text or raw_entity_lower in source_text:  # Sometimes the NER model won't tag the exact same thing in the original paragraph, but we can just do string matching
                continue
            ...
            if ent_new["type"] in ["DATE", "CARDINAL", "MONEY", "PERCENT"]:
                ...
                if ent_new["type"] == "CARDINAL":
                    if raw_entity_lower in self.string2digits and self.string2digits[raw_entity_lower] in source_text:
                        ...
                        raw_entity_lower] in source_text.replace(",", ""):
