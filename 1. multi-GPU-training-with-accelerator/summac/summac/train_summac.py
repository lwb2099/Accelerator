...
file_header = "1. multi-GPU-training-with-accelerator"

data.path.append(f"{file_header}/summac/nltk_data/")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="If not empty, will use the precomputed instead of "
    ...
    parser.add_argument(
        ...
        type=str,
        default="percentile",
        help="How should the bins of the histograms be "
    ...
    parser.add_argument(
        "--norm_histo",
        action="store_true",
        help="Normalize the histogram to be between 0 and 1, "
...
def train(
    ...
    if len(pre_file) == 0:
        standard_pre_file = f"{file_header}/summac_cache/train_%s_%s.jsonl" % (
            model,
            granularity,
        )
    ...
    save = False
    fcb = SummaCBenchmark(
        cut="train", dataset_names=["factcc"], accelerator=accelerator
    )
    ...
    model = SummaCConv(
        ...
        start_file=f"{file_header}/vitc_sentence_percentile_e_bacc0.884.bin",
        save=save,
        acc=accelerator,
    )
    ...
    if not silent:
        logger.info(
            "Length of dataset. [Train: %d]" % (len(d_train)), main_process_only=True
        )
    ...
    best_file = ""
    logger.info(
        f"start training on GPU={torch.cuda.get_device_name()}", main_process_only=True
    )
    ...
    for i, epoch in enumerate(range(num_epochs)):
        ...
        if not silent:
            itr = tqdm(
                itr, total=len(dl_train), disable=not accelerator.is_local_main_process
            )
            ...
            if (idx + 1) % eval_interval == 0:
                ...
                if val_score > best_val_score:
                    ...
                    if len(best_file) > 0 and accelerator.is_main_process:
                        os.remove(best_file)
                    best_file = f"{file_header}/%s_bacc%.3f.bin" % (experiment, best_val_score)
                    """convert to accelerator"""
...
if __name__ == "__main__":
    """
    accelerate launch --main_process_port 29508 --config_file ./summac_config.yaml ./summac/summac/train_summac.py --model vitc --granularity sentence --train_batch_size 8 --num_epochs 10 --nli_labels e
    """
    main()
